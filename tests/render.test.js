...
test("renderSessionLine prefers subscription plan over API env var", () => {
  ...
  try {
    ...
    assert.ok(
      !line.includes("API"),
      "should not include API label when plan is known",
    );
  } finally {
...
test("renderSessionLine shows syncing hint when usage API is rate-limited", () => {
  ...
  assert.ok(line.includes("usage:"), "should show usage label");
  assert.ok(
    line.includes("syncing..."),
    "should show syncing hint for rate limiting",
  );
  assert.ok(
    !line.includes("rate-limited"),
    "should not expose raw rate-limit error key",
  );
});
...
test("renderSessionLine keeps stale usage visible while rate-limited", () => {
  ...
  assert.ok(
    compactLine.includes("25%"),
    "should keep the last successful 5h usage visible",
  );
  assert.ok(
    compactLine.includes("85%"),
    "should keep the last successful 7d usage visible",
  );
  assert.ok(
    compactLine.includes("syncing..."),
    "should show syncing hint alongside stale usage",
  );

  const usageLine = renderUsageLine(ctx);
  assert.ok(
    usageLine?.includes("25%"),
    "expanded usage line should keep stale 5h usage visible",
  );
  assert.ok(
    usageLine?.includes("85%"),
    "expanded usage line should keep stale 7d usage visible",
  );
  assert.ok(
    usageLine?.includes("syncing..."),
    "expanded usage line should show syncing hint",
  );
});
...
test("renderSessionLine uses custom warning and critical colors for usage states", () => {
  ...
  assert.ok(
    warningLine.includes("\x1b[36musage: ⚠ (401)\x1b[0m"),
    `expected custom warning color, got: ${JSON.stringify(warningLine)}`,
  );
...
test("render expanded layout honors custom elementOrder including activity placement", () => {
  ...
  assert.ok(
    combinedIndex < environmentIndex,
    "environment line should follow context/usage",
  );
  assert.ok(
