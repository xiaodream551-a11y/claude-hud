 ...
 */
export function gitBranchColor(
  text: string,
  isDirty: boolean,
  branch: string,
): string {
  if (branch === "HEAD") return colorize(text, RED);
  if (isDirty) return colorize(text, YELLOW);
  return colorize(text, GREEN);
}
 ...
 */
export function speedColor(text: string, tokPerSec: number): string {
  let r, g, b;
...
export function getContextColor(
  ...
  if (gradient) return contextGradientAt(Math.min(percent, 100) / 100);
...
export function getQuotaColor(
  ...
  if (gradient) return quotaGradientAt(Math.min(percent, 100) / 100);
...
const EMPTY_START: [number, number, number] = [60, 60, 60];
const EMPTY_END: [number, number, number] = [30, 30, 30];
 ...
 */
function contextGradientAt(position: number): string {
  let r, g, b;
  if (position <= 0.7) {
 ...
 */
function quotaGradientAt(position: number): string {
  let r, g, b;
  if (position <= 0.75) {
...
function renderGradientBar(
  ...
  filledChar: string,
  emptyChar: string,
  gradientFn: (position: number) => string,
): string {
  let result = "";
  ...
  if (empty > 0 && filled > 0) {
    ...
    const lastRgb: [number, number, number] = match
